...
class Dynamic:
    ...
    # random serial dictatorship with processing
    # processing is the number of rounds that an agent can't except an item
    # randomly choose permutation of agents for selection each round
    def rsdWithProcessing(self):
            ...
            for round in tqdm(range(self.rounds)):
                ...
                processing_times = np.random.exponential(1, size=numAvailableAgents)
                # processing_times = np.random.uniform(0, 4, size=numAvailableAgents)
                # processing_times = np.ceil(processing_times)
...
dyn = Dynamic(rounds=500, agents=3, items=2, processing=2)
